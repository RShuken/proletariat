      ...
      if (repo.action === 'move') {
        ...
        if (isLocalPath(repo.path)) {
          ...
          if (remoteUrl) {
            ...
            console.log(chalk.yellow(`  Warning: No GitHub remote found on the source repository.`));
            console.log(chalk.yellow(`  The cloned repo's origin points to the local path: ${resolvedPath}`));
            console.log(chalk.yellow(`  To fix, add a remote: git -C "${targetPath}" remote set-url origin <github-url>`));
          }
        }
      }
...
/**
 * Exclude .devcontainer/ from git tracking via .git/info/exclude.
 * This prevents auto-generated devcontainer configs from making a repo
 * appear dirty after `prlt repo add`.
 */
function excludeDevcontainerFromGit(repoPath: string): void {
  try {
    ...
    if (!content.includes('.devcontainer/')) {
      ...
      content += `${suffix}# Auto-generated by prlt - exclude devcontainer configs from dirty detection\n.devcontainer/\n`;
      fs.writeFileSync(excludePath, content);
    }
  } catch {
    // Non-fatal: if we can't write exclude, dirty detection will still work
    // but the repo may show as dirty
  }
}
...
export function getRepoStatus(name: string, fullPath: string, dbRepo?: Repository): RepoInfo {
  ...
  try {
    ...
    // Check for uncommitted changes, excluding auto-generated .devcontainer/ files
    const statusRaw = execSync('git status --porcelain', {
      cwd: fullPath,
      stdio: 'pipe',
      encoding: 'utf-8'
    }).trim();
    // Filter out .devcontainer/ entries that prlt auto-generates (GH#815).
    // git status --porcelain format: 2 status chars + space + path (e.g. "?? .devcontainer/")
    const status = statusRaw
      .split('\n')
      .filter(line => line && !line.substring(3).startsWith('.devcontainer/'))
      .join('\n')
      .trim();
...
export async function addRepository(
  ...
  try {
    ...
    // doesn't show as dirty immediately after `prlt repo add` (GH#815)
    excludeDevcontainerFromGit(targetPath);
